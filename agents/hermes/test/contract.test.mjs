...
const DIRECT_ROUTE_MODULES = [
  ...
  ["../app/internal/mesh/actions/route.mjs", "POST"],
...
function validEnv(overrides = {}) {
  return {
    HERMES_AGENT_ID: "agent_22222222222222222222222222222222",
    MESH_ORIGIN: "https://mesh.example",
    MESH_AGENT_TOKEN: "mesh_test_agent_token",
    MESH_REGISTRATION_NONCE: "test-registration-nonce",
    AGENT_ORIGIN: "https://hermes.example",
    GATEWAY_INTERNAL_TOKEN: "hermes-worker-secret-with-enough-entropy",
    ...overrides,
  };
}
...
test("Hermes app exports a default Web Handler with distinct identity and skills", async () => {
  assert.equal(typeof defaultApp?.fetch, "function");
  const deps = dependencies();
  const app = createGatewayApp({ env: validEnv(), ...deps });
  const response = await app.fetch(
    new Request("https://hermes.example/.well-known/agent-card.json"),
  );
  const card = await response.json();

  assert.equal(response.status, 200);
  assert.equal(card.name, "Hermes");
  assert.equal(card.supportedInterfaces[0].url, "https://hermes.example/api/v1");
  assert.deepEqual(
    card.skills.map((skill) => skill.id),
    ["general-assistance", "agent-coordination", "direct-messages"],
  );
});

test("Hermes wrapper does not allow dependency injection to replace its identity", async () => {
...
test("Hermes app constructs its stateless MESH projection and introspection from injected dependencies", async () => {
  const deps = dependencies();
  ...
  const health = await app.fetch(new Request("https://hermes.example/health"));
  assert.equal(health.status, 200);
  assert.equal(deps.calls.bridge.recipientAgentId, validEnv().HERMES_AGENT_ID);
});

test("Hermes app validates required environment without exposing secrets to the client", () => {
...
test("Hermes app safely derives its origin from Vercel production URL", async () => {
  const deps = dependencies();
  const env = validEnv({
    AGENT_ORIGIN: "",
    VERCEL_PROJECT_PRODUCTION_URL: "hermes-production.vercel.app",
  });
  ...
  assert.equal(
    ...
    "https://hermes-production.vercel.app/api/v1",
  );
});

test("Hermes exposes every gateway endpoint through a direct Next.js route module", async () => {
...
test("Hermes removes obsolete private worker routes from source and built manifests", async () => {
...
test("Hermes inbound worker uses only canonical MESH mailbox configuration", async () => {
  ...
  assert.deepEqual(config, {
    ...
    agentIdEnv: "HERMES_AGENT_ID",
    pollIntervalMs: 15000,
    maxBackoffMs: 300000,
    runnerTimeoutMs: 600000,
    pageLimit: 1,
    runner: { module: "../../../packages/agent-worker/runners/hermes-runner.mjs" },
  });
  ...
  for (const variable of ["MESH_ORIGIN", "MESH_AGENT_TOKEN", "HERMES_AGENT_ID"]) {
...
test("Hermes outbound CLI selects compatibility credentials without coupling to mailbox config", async () => {
...
test("Hermes Vercel config uses Next.js without forbidden well-known rewrites", async () => {
...
test("Hermes environment template contains server-only gateway variables and no credentials", async () => {
  ...
  assert.equal(entries.HERMES_AGENT_ID, "");
  ...
  assert.doesNotThrow(() => createMeshActionHandler({ env: validEnv(), fetchImpl: async () => assert.fail("constructor must not fetch") }));
