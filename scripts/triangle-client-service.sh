...
for name in os.listdir(instances):
    ...
    if value["instanceId"] != expected or name != expected+".json" or value["runtimeAdapter"] not in {"codex","hermes","antigravity","grok-bot","cursor-acp"} or type(value["enabled"]) is not bool or value.get("deliveryMode", "worker") not in {"worker","mcp-interactive","event-driven","grok-bot","headless-app-server","headless-cursor-acp"}: raise SystemExit("invalid Triangle Client registry entry")
