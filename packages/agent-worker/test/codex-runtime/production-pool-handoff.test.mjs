...
test("TRIANGLE_CODEX_POOL_ENABLE raises production pool only when probe passed", () => {
  const profile = productionProfile();
  ...
  assert.equal(unproved.pool.preferredSize, 1);
  assert.equal(unproved.pool.maxSize, 1);
  assert.equal(unproved.pool.forcedByProbe, true);
  assert.equal(unproved.pool.productionPoolInactiveReason, "shared_home_concurrency_unproved");
