...
public enum ClientInstanceContractCases {
    ...
    public static func deliveryModeRoundTrip() throws {
        try withStore { store, _ in
            ...
            let instance = try ClientInstance(profile: profile, runtimeAdapter: .codex)
            try store.create(instance)
            try expect(try store.read(profile: profile).deliveryMode == .headlessAppServer, "new Codex instance did not default to headless-app-server")
            try expect(try store.read(profile: profile).participatesInHeadlessWake, "new Codex instance is not in headless wake")
            try expect(try store.read(profile: profile).participatesInCursorAcpWake == false, "new Codex instance incorrectly claims Cursor ACP wake")
            try expect(try store.read(profile: profile).participatesInGrokBotWake == false, "new Codex instance incorrectly claims Grok Bot wake")
            ...
            try store.setDeliveryMode(.headlessAppServer, profile: profile)
            try expect(try store.read(profile: profile).deliveryMode == .headlessAppServer, "headless-app-server delivery mode did not round trip")
            try expect(try store.read(profile: profile).participatesInWorkerPolling == false, "headless profile still participates in worker polling")
            try expect(try store.read(profile: profile).participatesInEventDrivenWake == false, "headless profile incorrectly claims event-driven wake")
            try expect(try store.read(profile: profile).participatesInAppServerWake == false, "headless profile incorrectly claims App Server wake")
            try expect(try store.read(profile: profile).participatesInGrokBotWake == false, "headless profile incorrectly claims Grok Bot wake")
            try expect(try store.read(profile: profile).participatesInHeadlessWake, "headless profile does not participate in headless wake")
            try expect(try store.read(profile: profile).participatesInWatchGrantNotify == false, "headless profile incorrectly joined watch grant notify")
            let grok = try ProfileName("bob")
            try store.create(ClientInstance(profile: grok, runtimeAdapter: .grokBot))
            try expect(try store.read(profile: grok).deliveryMode == .grokBot, "new grok-bot instance did not default to grok-bot delivery")
            try expect(try store.read(profile: grok).participatesInGrokBotWake, "new grok-bot instance is not on grokBotWake")
            try expect(try store.read(profile: grok).participatesInHeadlessWake == false, "grok-bot entered the Codex headless pool")
            try expect(try store.read(profile: grok).participatesInCursorAcpWake == false, "grok-bot entered the Cursor ACP lane")
            try store.setDeliveryMode(.headlessAppServer, profile: grok)
            try expect(try store.read(profile: grok).participatesInHeadlessWake == false, "grok-bot adapter still joined headless wake")
            let cursor = try ProfileName("cursor-acp-shadow-test")
            try store.create(ClientInstance(profile: cursor, runtimeAdapter: .cursorAcp))
            try expect(try store.read(profile: cursor).deliveryMode == .headlessCursorAcp, "new cursor-acp instance did not default to headless-cursor-acp")
            try expect(try store.read(profile: cursor).participatesInCursorAcpWake, "new cursor-acp instance is not on cursorAcpWakes")
            try expect(try store.read(profile: cursor).participatesInHeadlessWake == false, "cursor-acp entered the Codex headless pool")
            try expect(try store.read(profile: cursor).participatesInGrokBotWake == false, "cursor-acp entered grokBotWake")
            try store.setDeliveryMode(.headlessAppServer, profile: cursor)
            try expect(try store.read(profile: cursor).participatesInHeadlessWake == false, "cursor-acp adapter still joined Codex headless wake")
            try expect(try store.read(profile: cursor).participatesInCursorAcpWake == false, "mis-set deliveryMode still claimed Cursor ACP wake")
