...
struct WatchGrantTests {
    ...
    @Test("keeps local binding when stale recreate fails")
    func failedStaleRecreateKeepsLocalBinding() async throws {
        try await WatchGrantContractCases.failedStaleRecreateKeepsLocalBinding()
    }
